      ...
      if ($_GET["tab"] == "info") {
        if (isset($_GET['act'])) {
          if ($_GET["act"]) {
            ...
            $sql_get_info_prj = "SELECT *,DATE_FORMAT( dateStart,  '%d/%m/%Y' ) AS dateStart FROM project_info a, lab_info b, user_info c WHERE (a.idLab = b.idLab) AND (a.nameUser = c.fullName) AND a.idProject = '$id'";
            ...
            if ($db->num_rows($sql_get_full_prj)) //Dự án có nhiều người tham gia
            {
               ...
               echo '<div class="container">
                  ...
                  <span>Chủ nhiệm: <a href="'.$_DOMAIN.'profile/'.$data_prj_info['idUser'].'">'.$data_prj_info['nameUser'].'</a></span><br />
                  <span>Hướng dẫn: '.$data_prj_info['nameStaff'].'</span><br />
                  <span>Lab tham gia: <a href="'.$_DOMAIN.'labs/info/'.$data_prj_info['idLab'].'">'.$data_prj_info['nameLab'].'</a></span><br />
                  ...
                  if ($total_device_using > 0) {
                    foreach ($db->fetch_assoc($sql_get_device, 0) as $key => $value_device) {
                      echo '- <a href="'.$_DOMAIN.'device/info/'.$value_device['idDevice'].'">'.$value_device['nameDevice'].'</a> - '.$value_device['totalBorrow'].'<br  />';
                    }
              ...
              echo '<div class="container">
                 ...
                 if ($total_device_using > 0) {
                   foreach ($db->fetch_assoc($sql_get_device, 0) as $key => $value_device) {
                     echo '- <a href="'.$_DOMAIN.'device/info/'.$value_device['idDevice'].'">'.$value_device['nameDevice'].'</a> - Số lượng: '.$value_device['totalBorrow'].'<br  />';
                   }
                 } else echo '<br><div class="alert alert-info">Chưa sử dụng thiết bị nào.</div>';
                 // <a href="'.$_DOMAIN.'#">- '.$data_lab_info['nameProject'].' - '.$data_lab_info['nameUser'].'</a><br  />
             echo '</div>
